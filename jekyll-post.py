...
-d: the default date is today's date or this option can be used to specify a 
    date. The date must be specified in the format YYYY-MM-DD.
-n: the name for the post file instead of using the automatically generated one.
-t: the tags that should be added to the yaml portion of the file separated by 
    spaces and the whole thing wrapped in quotes.
-w: this optional flag will signal the file to be opened for writing in NeoVim.
yaml options:
---
layout: post
comments: false
...
def main():
    ...
    parser.add_argument('-D', '--Dir', help='specify the directory (relative or absolute) that the file should be written to.', default='./_posts/')
    ...
    # if write flag is on, then try to open file with NeoVim
    if args.write:
